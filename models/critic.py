...
class EvaluationResult:
    ...
    @property
    def predicted_logits_np(self) -> np.ndarray:
        return self.predicted_logits.detach().cpu().numpy()

    @property
    def predicted_probs(self):
        return F.softmax(self.predicted_logits, dim=-1)

    @property
    def predicted_probs_np(self):
        return self.predicted_probs.detach().cpu().numpy()

    @property
    def target_labels_np(self):
        if self.target_labels is None:
            return None
        else:
            return self.target_labels.detach().cpu().numpy()
        
class Accumulator(Metric):
    def __init__(self) -> None:
        super().__init__(compute_on_step=True, dist_sync_on_step=False)
        self.add_state("predicted_logits", default=torch.FloatTensor([]), dist_reduce_fx="min")
        self.add_state("target_labels", default=torch.IntTensor([]), dist_reduce_fx="min")
        self.add_state("loss", default=torch.tensor(0.0), dist_reduce_fx="sum")

    def update(self, predicted_logits: torch.Tensor, target_labels: Optional[torch.Tensor] = None, loss: Optional[torch.Tensor] = None):
        self.predicted_logits = torch.cat([self.predicted_logits, predicted_logits], dim=0)
    ...
    def compute(self):
        return EvaluationResult(
            ...
            self.target_labels if self.target_labels.numel() > 0 else None,
            (self.loss / self.target_labels.shape[0]) if self.target_labels.numel() > 0 else None,
        )
...
class CriticDataModule(pl.LightningDataModule):
    ...
    args: Any
    ds: Optional[Mapping[str, List]] = None
...
class TransformerCritic(BaseTransformer):
    mode = "sequence-classification"
    def __init__(self, hparams):
        if isinstance(hparams, dict):
            hparams = Namespace(**hparams)

        self.processor = processors[hparams.task_name](hparams.task_name, max_history=hparams.max_history)

        if hparams.force_reinit:
            num_classes_old = AutoConfig.from_pretrained(hparams.config_name_or_path).num_labels
        elif hparams.config_name_or_path:
            num_classes_old = AutoConfig.from_pretrained(hparams.config_name_or_path).num_labels
    ...
    def validation_epoch_end(self, outputs):
        validation_loss, metrics, probabilities, _ = self._eval_end("valid")
        self.log("valid/loss", validation_loss, prog_bar=True, logger=True)
    ...
    def test_epoch_end(self, outputs):
        test_loss, test_metrics, probabilities, out_labels = self._eval_end("test")
        ...
        if not self.use_ddp or dist.get_rank() == 0:
            self._save_predictions(probabilities, mode, examples)
        ...
        if self.use_ddp and self.trainer.num_gpus > 1:
            ...
            if dist.get_rank() == 0:
                all_tensors = torch.empty(
                    tensor.shape[0] * dist.get_world_size(),
    ...
    def _save_predictions(self, probabilities, mode: str, examples: list):
        output_file = f"{mode}"
        ...
        with open(preds_file, "w") as writer:
            for i, ex in enumerate(examples):
                example_json = dict(ex)
                p = probabilities[i].tolist()
                example_json["probs"] = p
                example_json["predicted_label"] = self.processor.labels[np.argmax(p)]
                predicted_freq[example_json["predicted_label"]] += 1

                writer.write(json.dumps(example_json) + "\n")

        if not self.hparams.do_train:
            rank_zero_info(f"predictions saved in `{preds_file}`")
    @staticmethod
    def add_args_for_model(parser):
        BaseTransformer.add_args_for_model(parser)
        parser.add_argument(
            "--max_history",
            type=int,
            default=1,
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Whether to force reinitializing the head.",
        )
        parser.add_argument(
            "--test_task",
            default="FaithCritic",
    ...
    def get_default_output_dir(args: Namespace) -> str:
        ...
        output_dir_name = (
            f"{model_name}_{task}_ep{args.max_epochs}_bsz{args.train_batch_size}x{num_gpus}"
        )
        if args.warmup_ratio > 0:
            output_dir_name += f"_wmr{args.warmup_ratio}"
        else:
            output_dir_name += f"_wm{args.warmup_steps}"
        ...
        if args.save_last:
            output_dir_name += "_last"

        if args.fp16:
            output_dir_name += "_amp"
...
def main():
    ...
    TransformerCritic.add_model_specific_args(parser)
